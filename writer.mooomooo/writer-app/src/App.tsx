...
function App() {
  const { currentDocument, createDocument, loadDocuments, login, editorMode, setEditorMode, isLoading } = useWriterStore();
    ...
    const initializeApp = async () => {
      try {
        ...
        if (documents.length === 0) {
          await createDocument('Welcome to Writer');
          ...
          setTimeout(() => {
            ...
            addParagraphWithoutHistory('- **Cloud Storage**: Your documents are automatically saved to the cloud using MinIO (S3-compatible)');
  ...
  const handleLoginSuccess = (credential: string) => {
    try {
      ...
      toast({
        title: 'Login failed',
        description: 'Invalid credentials received from Google',
        variant: 'error',
      });
    }
  };
